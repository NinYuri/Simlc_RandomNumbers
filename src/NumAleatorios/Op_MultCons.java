...
public class Op_MultCons extends JFrame implements ActionListener
{
	...
	private JTextField txtcons = new JTextField(6);
	private JLabel lbln = new JLabel("Iteraciones:");
	private JLabel lblsem = new JLabel("Semilla:");
	private JLabel lblcons = new JLabel("Constante:");
	...
	public Op_MultCons()
	{
		...
		paneltop.add(lblsem);
		paneltop.add(txtsem);
		paneltop.add(lblcons);
		paneltop.add(txtcons);
	...
	{	
		...
		if(e.getSource() == bttgenerar || e.getSource() == bttregre)
		{
			...
			if(sem.isEmpty())
			{
				...
				if(sem.length() != 4)
				{
					JOptionPane.showMessageDialog(this, "La semilla debe tener, por lo menos, 4 dígitos", 
							"Error de semilla", JOptionPane.ERROR_MESSAGE);
					txtsem.setText("");
					return;
				}
			...
			if(cons.isEmpty())
			{
				JOptionPane.showMessageDialog(this, "Debe ingresar una constante", 
						"Error de constante", JOptionPane.ERROR_MESSAGE);
				return;
			}
			else
				if(cons.length() != 4)
				{
					JOptionPane.showMessageDialog(this, "La constante debe tener, por lo menos, 4 dígitos", 
							"Error de constante", JOptionPane.ERROR_MESSAGE);
					txtcons.setText("");
					return;
				}
					...
					{
						JOptionPane.showMessageDialog(this, "La constante debe ser un número entero", 
								"Error de constante", JOptionPane.ERROR_MESSAGE);
						txtcons.setText("");
						return;
					}
			...
			int constante = Integer.parseInt(cons);
			...
			if(e.getSource() == bttmed)
			{
				if(n.isEmpty())
				{
					...
					if(sem.isEmpty())
					{
						...
						if(cons.isEmpty())
						{
							JOptionPane.showMessageDialog(null, "Debe ingresar una constante", 
									"Error de prueba de media", JOptionPane.ERROR_MESSAGE);
							return;
						}
						else
