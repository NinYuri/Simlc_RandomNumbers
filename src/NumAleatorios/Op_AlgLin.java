...
public class Op_AlgLin extends JFrame implements ActionListener
{
	...
	private JTextField txtsem = new JTextField(4);
	private JTextField txta = new JTextField(4);
	private JTextField txtm = new JTextField(4);
	...
	private JLabel lbla = new JLabel("Multiplicador constante:");
	private JLabel lblm = new JLabel("Potencia del módulo:");
	private JLabel lblc = new JLabel("Incremento:");
	...
	{	
		...
		String cons = txta.getText();
		String pot = txtm.getText();
		...
		if(e.getSource() == bttgenerar || e.getSource() == bttregre)
		{
			...
			if(cons.isEmpty())
			{
				JOptionPane.showMessageDialog(this, "Debe ingresar un multiplicador para la constante", 
						"Error de multiplicador constante", JOptionPane.ERROR_MESSAGE);
				return;
			}
			else
				if(Entero(cons) == false)
				{
					JOptionPane.showMessageDialog(this, "El multiplicador constante debe ser un número entero", 
							"Error de multiplicador constante", JOptionPane.ERROR_MESSAGE);
					txta.setText("");
					return;
				}
				else
					if(Integer.parseInt(cons) <= 0)
					{
						JOptionPane.showMessageDialog(this, "El multiplicador constante debe ser mayor a 0", 
								"Error de multiplicador constante", JOptionPane.ERROR_MESSAGE);
						txta.setText("");
						return;
					}
			...
			if(c.isEmpty())
			{
				JOptionPane.showMessageDialog(this, "Debe ingresar un incremento", 
						"Error de incremento", JOptionPane.ERROR_MESSAGE);
				return;
			}
			else
				if(Entero(c) == false)
				{
					JOptionPane.showMessageDialog(this, "El incremento debe ser un número entero", 
							"Error de incremento", JOptionPane.ERROR_MESSAGE);
					txtc.setText("");
					return;
				}
				else
				{
					...
					{
						JOptionPane.showMessageDialog(this, "El incremento debe ser mayor a 0", 
								"Error de incremento", JOptionPane.ERROR_MESSAGE);
						txtc.setText("");
						return;
					}
					if(Comparar(Integer.parseInt(c), Integer.parseInt(pot)) != true)
					{
						JOptionPane.showMessageDialog(this, "El incremento debe ser un primo relativo de " + (int)Math.pow(2,Integer.parseInt(pot)), 
								"Error de incremento", JOptionPane.ERROR_MESSAGE);
						txtc.setText("");
						return;
					}
			...
			int mult = Integer.parseInt(cons);
			int potencia = Integer.parseInt(pot);
			int inc = Integer.parseInt(c);
			int a, m, xn;
			double na, ri, alet;
			String ric;
	...
	private static int CompMCD(int m, int c) 
	{
        while (c != 0)
        {
            int temp = c;
	...
	private boolean Comparar(int c, int pot)
	{
		int m = (int)Math.pow(2, pot);
		int mcd = CompMCD(m, c);
		if (mcd == 1)
			return true;
		else
