...
public class Op_CongMult extends JFrame implements ActionListener
{
	...
	private JTextField txtcon = new JTextField(4);
	...
	private JLabel lblcon = new JLabel("Multiplicador constante:");
			...
			if(sem.isEmpty())
			{
				...
				if(Entero(sem) == false)
				{
					...
					if(Integer.parseInt(sem) <= 0)
					{
						...
						if(Impar(Integer.parseInt(sem)) == false)
						{
							JOptionPane.showMessageDialog(this, "La semilla debe ser un número impar", 
									"Error de semilla", JOptionPane.ERROR_MESSAGE);
							txtsem.setText("");
							return;
						}
	...
	private static boolean Impar(int sem)
	{
		if(sem % 2 != 0)
			return true;
		else
